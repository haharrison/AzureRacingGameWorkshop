...
using AzureRacingGameWorkshop.AzureData;
#endregion
...
namespace RacingGame.GameLogic
{
    ...
    public class Player : ChaseCamera
    {
        ...
        public Player(Vector3 setCarPosition)
            : base(setCarPosition)
        {
            m_LapTimeManager = new LapTimeManager();
        }
        #endregion
        ...
        private void SendTelemetry()
        {
            ...
            if (m_LastTelemetryLog + Settings.LogTelemetryInterval < DateTime.UtcNow)
            {
                if (currentGameTimeMilliseconds > 100)
                {
                    //Debug.WriteLine("LogTelemetryInterval");
                ...
                // Do we need to send telemetry
                if (m_LastTelemetrySend + Settings.SendTelemetryInterval < DateTime.UtcNow)
                {
                    //Debug.WriteLine("SendTelemetryInterval");
                   
                  
                    
                    m_LastTelemetrySend = DateTime.UtcNow;
                ...
                m_LastTelemetryLog = DateTime.UtcNow;
            }

            // Do we need to send car position?
            //if (m_LastPositionSend + Constants.SendPositionInterval < DateTime.UtcNow)
            //{
            //    // Send the car posiiton
            ...
            //    if (GameSettings.SendData)
            //    {
            //        m_CarPositionQueue.AddCarPositionDataAsync(carPositionData);
            //    }
            //    m_LastPositionSend = DateTime.UtcNow;
            //}

            //if (m_LastTelemetryUpdate + TelemetryUpdateInterval < DateTime.UtcNow)
            //{
            ...
            //    CarY = CarPosition.Y,
            //    CarZ = CarPosition.Z,
            //    Level = TrackSelection.SelectedTrackNumber,
            //    HasAccelerated = hasAccelerated,
            //    HasBraked = hasBreaked,
            //    LapComplete = false,
            //    LapTimeMs = 0,
            //    CurrentGameTimeMs = (int)currentGameTimeMilliseconds,
            //    TrackSegmentNumber = trackSegmentNumber,
            //    TrackSegmentPercent = trackSegmentPercent,
            //    WheelPos = CarWheelPos
            //};


            //m_TelemetryManager.EnqueueDataAsync(data);
            ...
            //m_TelemetryManager.EnqueueData(data);
            //m_TelemetryManager.EnqueueDataAsync(data);


            ////this.GameTimeMilliseconds
            //if (this.GameTimeMilliseconds > 10000)
            //{
            //    int i = 0;
            //}
            ...
            //Debug.WriteLine("Update in game: " + Speed);

            //m_LastTelemetryUpdate = DateTime.UtcNow;

        }
        #endregion

        //public void HandleTelemertyReplayAndCheckpoints()
        //{
        //    if (trackSegmentNumber )
        //}
