...
using AzureRacingGameWorkshop.AzureData.Blobs;
#endregion
...
namespace RacingGame.GameLogic
{
    ...
    public class Replay : ICloneable
    {
        ...
        static ReplayBlobStore ReplayBlobStore = new ReplayBlobStore();
        ...
        /// </summary>
        public static readonly string[] ReplayFilenames = new string[]
            {
        ...
        {
            //*** We find the appropriate track time (but try to do it better than this :-)
            while(lapTime > 0 && trackTime > lapTime)
            {
                trackTime -= lapTime;
            }
        ...
        public Replay(int setTrackNum, bool createNew, Track track)
        {
            ...
            //throw new NotImplementedException("We can't do this!!");
        ...
        public Replay(string playerName, string blobName, int trackNumber)
        {
            PlayerName = playerName;

            trackNum = trackNumber;

            //CarNumber = RandomHelper.GetRandomInt(3);
            CarNumber = 2;
            Color = RandomHelper.RandomColor;

            try
            {
                //ReplayBlobStore blobStore = new ReplayBlobStore();

                using (Stream stream = ReplayBlobStore.DownloadStream(blobName))
                {
                    using (BinaryReader reader = new BinaryReader(stream))
                    {
                        // Load total lap time
        ...
        public void SaveToBlob()
        {
            string blobName = ReplayFilenames[trackNum].Replace (".Replay", "-" + RacingGameManager.Player.m_LapId + ".Replay");
            ...
            using (MemoryStream stream = new MemoryStream())
            {
                using (BinaryWriter writer = new BinaryWriter(stream))
                {
                    ...
                    //ReplayBlobStore blobStore = new ReplayBlobStore();
                    ReplayBlobStore.UploadStream(blobName, stream);
                }
            }
