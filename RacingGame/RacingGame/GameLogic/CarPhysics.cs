...
namespace RacingGame.GameLogic
{
    ...
    public class CarPhysics : BasePlayer
    {
        ...
        /// </summary>
        public float trackSegmentPercent = 0;
        ...
        public override void Update()
        {
            ...
            m_BrakesPercent = 0.0f;
            m_ThrottlePercent = 0.0f;
            ...
            //    return;
            if (RacingGameManager.Player.Mode == ChaseCamera.CameraMode.FreeCamera)
                return;
            ...
            if (Input.MouseXMovement != 0)
                rotationChange -= effectiveSensitivity * (Input.MouseXMovement / 15.0f) * MaxRotationPerSec * moveFactor;

            if (Input.IsGamePadConnected)
            {
                // More dynamic force changing with gamepad (slow, faster, etc.)
                rotationChange -= effectiveSensitivity * Input.GamePad.ThumbSticks.Left.X * MaxRotationPerSec * moveFactor / 1.12345f;
            ...
            if (Input.IsGamePadConnected)
            {
                m_ThrottlePercent = Input.GamePad.Triggers.Right;
                m_BrakesPercent = Input.GamePad.Triggers.Left;
            }
            ...
            if (Input.KeyboardUpPressed || Input.Keyboard.IsKeyDown(Keys.W) || Input.MouseLeftButtonPressed || Input.GamePadAPressed)
            {
                newAccelerationForce += maxAccelerationPerSec;// * moveFactor;
                m_ThrottlePercent = 1.0f;
            }

            // Down or right mouse button decelerates
            else if (Input.KeyboardDownPressed || Input.Keyboard.IsKeyDown(Keys.S) || Input.Keyboard.IsKeyDown(Keys.O) || Input.MouseRightButtonPressed)
            {
                newAccelerationForce -= maxAccelerationPerSec;// * moveFactor;
                m_BrakesPercent = 1.0f;
            }

            else if (Input.IsGamePadConnected)
            {
                // More dynamic force changing with gamepad (slow, faster, etc.)
                newAccelerationForce += (Input.GamePad.Triggers.Right) * maxAccelerationPerSec;// *moveFactor;
                ...
                RacingGameManager.InGame && !GameOver)
            {
                ...
                else
                {
                        ...
                        trackSegmentNumber)
                    {
                        ...
                        //*** Show position
                        Tuple<int, int> sectorPosition = m_LapTimeManager.GetSectorPosition(this.LevelNum, num, (int)GameTimeMilliseconds);
                        int sectorTimeDifference = (int)GameTimeMilliseconds - sectorPosition.Item2;
                        BaseGame.UI.AddSectorTimeFadeupEffect(num + 1, sectorPosition.Item1, sectorTimeDifference);
                        ...
                        //*** Add a segment to the lap data
                        CurrentLapData.SectorTimesMs.Add((int)RacingGameManager.Player.GameTimeMilliseconds);
        ...
        public Matrix UpdateCarMatrixAndCamera()
        {
            ...
            else if (RacingGameManager.Player.Mode == ChaseCamera.CameraMode.FreeCamera)
            ...
            if (RacingGameManager.Player.GameTimeMilliseconds > RacingGameManager.Landscape.NewReplay.NumberOfTrackMatrices * Replay.TrackMatrixIntervals * 1000.0f)
            {
