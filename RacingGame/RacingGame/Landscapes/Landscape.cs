...
namespace RacingGame.Landscapes
{
    ...
    {
        Dictionary<string, Replay> m_ReplayCache;


        public Dictionary<string, Replay> ReplayCache
        {
            get
            {
        ...
        public void StartNewLap()
        {
            ...
            //*** Always save the blob.
            RacingGameManager.Landscape.NewReplay.CheckpointTimes.Add(thisLapTime);
            newReplay.LapTime = thisLapTime;
            ThreadPool.QueueUserWorkItem(new WaitCallback(SaveReplay),
                                            (Replay)newReplay.Clone());
        ...
        {
            m_ReplayCache = new Dictionary<string, Replay>();
            ...
            //*** Load all 3 tracks
            track0 = new Track("Track" + RacingGameManager.Level.Beginner.ToString(), this);
            track1 = new Track("Track" + RacingGameManager.Level.Advanced.ToString(), this);
            track2 = new Track("Track" + RacingGameManager.Level.Expert.ToString(), this);
        ...
        internal void ReloadLevel(RacingGameManager.Level setLevel)
        {
            ...
            for (int ghostCarLevel = 0; ghostCarLevel < 3; ghostCarLevel++)
            {
                List<LapData> topLaps = RacingGameManager.Player.LapTimeManager.LapTimes[ghostCarLevel];


                foreach (LapData topLap in topLaps)
                {
                    if (!m_ReplayCache.ContainsKey(topLap.LapId) && m_ReplayCache.Count < Settings.MaxGhostCars)
                    {
                        string blobName = Replay.ReplayFilenames[ghostCarLevel].Replace(".Replay", "-" + topLap.LapId + ".Replay");
                        Debug.WriteLine("Loading replay: " + blobName);
                        m_ReplayCache.Add(topLap.LapId, new Replay(topLap.PlayerName, blobName, ghostCarLevel));
                    }
                }
            }
        ...
        public void GenerateShadow()
        {
            ...
            track0.GenerateShadow();
            track1.GenerateShadow();
            track2.GenerateShadow();
