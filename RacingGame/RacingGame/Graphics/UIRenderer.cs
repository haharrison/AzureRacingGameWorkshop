...
namespace RacingGame.Graphics
{
    ...
    public class UIRenderer : IDisposable
    {
        ...
        public void AddSectorTimeFadeupEffect(int sector, int position, int sectorTimeDifference)
        {
            //string text = "Sector: " + sector;
            string text = "Position: " + position.ToString();

            Color col = Color.White;
            if (sectorTimeDifference > 0)
            {
                text += " +";
                col = Color.Red;
            }
            else if (sectorTimeDifference < 0)
            {
                text += " -";
                col = Color.Green;
            }
            ...
            text +=
                //min
                ((sectorTimeDifference / 1000) / 60) + ":" +
                //sec
                ((sectorTimeDifference / 1000) % 60).ToString("00") + "." +
                //ms
                ((sectorTimeDifference / 10) % 100).ToString("00");
        ...
        public void RenderMenuTrackBackground()
        {
            ...
            if (RacingGameManager.InCarSelectionScreen == false)
            {
                ...
                foreach (KeyValuePair<string, Replay> replayKvps in RacingGameManager.Landscape.ReplayCache)
                {
                    ...
                    Matrix replayCarMatrix = replayKvps.Value.GetCarMatrixAtTime((float)RacingGameManager.ApplicationTime.TotalMilliseconds / 1500.0f);
                    //Matrix replayCarMatrix = replayKvps.Value.GetCarMatrixAtTime(20.0f);


                    replayCarMatrix = Matrix.CreateRotationX(MathHelper.Pi / 2.0f) * Matrix.CreateRotationZ(MathHelper.Pi) * replayCarMatrix;
                    RacingGameManager.CarModel.RenderCar(replayKvps.Value.CarNumber, replayKvps.Value.Color, false, replayCarMatrix);

                    //Debug.WriteLine("" + replayKvps.Value.CarNumber + "-" + replayKvps.Value.Color.R + "," + replayKvps.Value.Color.G + "," + replayKvps.Value.Color.B);
                }
                //Debug.WriteLine("");
        ...
        {
            //Debug.WriteLine("UpdateCarInMenu");


            carMenuTime += BaseGame.ElapsedTimeThisFrameInMilliseconds / 1000.0f;

            // Use data from replay
            Matrix carMatrix = RacingGameManager.Landscape.BestReplay.GetCarMatrixAtTime(carMenuTime);
            carPos = carMatrix.Translation;

            
            RacingGameManager.Player.SetCarPosition(carPos, carMatrix.Forward, carMatrix.Up);
            ...
            //Vector3 camPos = carPos + oldCarForward * 13 - oldCarUp * 1.3f;
            //camPos.Z -= 100;

            //Vector3 camPos2 = new Vector3(1200, 1200, -500);

            //RacingGameManager.Player.SetCameraPosition(camPos2);
            ...
            string trackName, int[] top5LapTimes)
        {
            ...
            Rectangle lapsRect = BaseGame.CalcRectangle1600(10, 10, LapsGfxRect.Width, LapsGfxRect.Height);
            ingame.RenderOnScreen(lapsRect, LapsGfxRect, baseUIColor);

            Rectangle timesRect = BaseGame.CalcRectangle1600(10, 10, CurrentAndBestGfxRect.Width, CurrentAndBestGfxRect.Height);
            timesRect.Y = BaseGame.Height - timesRect.Bottom;
            ingame.RenderOnScreen(timesRect, CurrentAndBestGfxRect, baseUIColor);

            Rectangle trackNameRect = BaseGame.CalcRectangle1600(10, 10, TrackNameGfxRect.Width, TrackNameGfxRect.Height);
            trackNameRect.X = BaseGame.Width - trackNameRect.Right;
            ingame.RenderOnScreen(trackNameRect, TrackNameGfxRect, baseUIColor);

            Rectangle top5Rect1 = BaseGame.CalcRectangle1600(10, 4, Best5GfxRect.Width, Best5GfxRect.Height);
            ...
            Rectangle top5Rect2 = new Rectangle(top5Rect1.X, top5Rect1.Bottom + top5Distance, top5Rect1.Width, top5Rect1.Height);
            ingame.RenderOnScreen(top5Rect2, Best5GfxRect, baseUIColor);

            Rectangle top5Rect3 = new Rectangle(top5Rect1.X, top5Rect2.Bottom + top5Distance, top5Rect1.Width, top5Rect1.Height);
            ingame.RenderOnScreen(top5Rect3, Best5GfxRect, baseUIColor);

            Rectangle top5Rect4 = new Rectangle(top5Rect1.X, top5Rect3.Bottom + top5Distance, top5Rect1.Width, top5Rect1.Height);
            ingame.RenderOnScreen(top5Rect4, Best5GfxRect, baseUIColor);

            Rectangle top5Rect5 = new Rectangle(top5Rect1.X, top5Rect4.Bottom + top5Distance, top5Rect1.Width, top5Rect1.Height);
            ingame.RenderOnScreen(top5Rect5, Best5GfxRect, baseUIColor);



            Rectangle tachoRect = BaseGame.CalcRectangle1600(10, 10, TachoGfxRect.Width, TachoGfxRect.Height);
 ...
 false;
        ...
        public void RenderTextsAndMouseCursor()
        {
            ...
            if (showFps)
                ...
                //TextureFont.WriteText(BaseGame.XToRes(200), BaseGame.YToRes(26), "Fps: " + RacingGameManager.Player.trackSegmentNumber + " " + BaseGame.Width + "x" + BaseGame.Height);
