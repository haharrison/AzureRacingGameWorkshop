...
using AzureRacingGameWorkshop.AzureData.DataContracts;
using System.IO;
using System.Xml;
...
namespace RacingGame.Tracks
{
    ...
    public class TrackLine
    {
            ...
            Landscape landscape)
        {
            ...
            Debug.WriteLine("");
            Debug.WriteLine("Track line");

            TrackVectorData data = new TrackVectorData();
            ...
            for (int num = 0; num < inputPoints.Length; num++)
            {
                ...
                for (int iter = 0; iter < numberOfIterations; iter++)
                {
                    ...
                    data.TrackPoints.Add(new TrackVector() { X = newVertex.pos.X, Y = newVertex.pos.Y, Z = newVertex.pos.Z });
                    //Debug.WriteLine("    <point x=\"" + newVertex.pos.X + "\" y=\"" + newVertex.pos.Y + "\" z=\"" + newVertex.pos.Z + "\" />");
                }
            }


            //string fileName = @"C:\Temp\Track" + Guid.NewGuid().ToString() + ".xml";
            ...
            //DataContractSerializer dcs = new DataContractSerializer(typeof(TrackVectorData));


            //using (Stream stream = new FileStream(@"C:\Temp\Track" + Guid.NewGuid().ToString() + ".xml", FileMode.Create, FileAccess.Write))
            //{
